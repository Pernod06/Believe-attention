...
def train():
    ...
    optimizer = optim.RMSprop(filter(lambda p: p.requires_grad, lprnet.parameters()), lr=args.learning_rate, alpha = 0.9, eps=1e-08,
                         momentum=args.momentum, weight_decay=args.weight_decay)
    # train_img_dirs = os.path.expanduser(args.train_img_dirs)
    # test_img_dirs = os.path.expanduser(args.test_img_dirs)
    
    train_dataset = LPRDataLoader(args.train_img_dirs, args.img_size, args.lpr_max_len)
    ...
    for iteration in tqdm(range(start_iter, max_iter),desc="Training"):
